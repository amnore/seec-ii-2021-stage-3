...
|                              | **前置条件** 用户有权限查看该课程贴子，参见 SRS1             |
|                              | **后置条件** 返回是否成功发布评论的消息，若成功将该条comment写入数据库，若失败，显示错误信息 |
| CommentService.deleteComment | **语法** `public ResultVO<CommentVO> deleteComment(CommentVO comment)` |
|                              | **前置条件** 该条评论由删除者创建                            |
|                              | **后置条件** 返回是否成功删除评论的消息，若成功将该条comment从数据库中删除，若失败，显示错误信息 |
...
![](https://ftp.bmp.ovh/imgs/2021/06/0ade0caf66546367.jpg)
...
\- replyType: type = Integer[为1指回应的是post，为0指回应的是comment]

\- receiverID: type = integer[回应的若是评论，receiver指的是回应的评论的commentID；否则若回应的是postID，该字段指postID]

\- authorID: type = Integer[写该条评论的用户id]
